...
Plotting the data in (D) as fold enrichment over non-NMD genes shows that the fraction of genes significantly upregulated by CHX among genes containing regulated AS-NMD events tends to grow during neurodevelopment. Consistent with (C), this increase is especially prominent for NS-CE-containing genes.

```{r}
# set colours

line.dot.colours <- c(`no-NMD` = "#b5b5b5", `NR-CE` = "#D65C74",
                  `NS-CE` = "#3184BC", `NR-other`= "#C1A4B6", 
                  `NS-other` = "#8BAEB7")
...
## Plot fold-enrichment of CHX-upregulated genes
### calculate fold-enrichment
dat.foldenr <- dat.tally %>% 
    group_by(Timepoint) %>% 
    filter(dge.CHX.up) %>% 
    mutate(fisher.est = frac/frac[1])

labels <- dat.foldenr %>% 
    filter(Timepoint == "Day 24") %>% 
    mutate(fisher.est = ifelse(groupings == "NS-other", fisher.est - 0.2, fisher.est + 0.2))

fig1e <- dat.foldenr %>% 
    ggplot(aes(x=Timepoint, y = fisher.est, colour = groupings)) +
    geom_line(aes(group=groupings), size = 1) +
    geom_point(size = 2.5) +
    theme_classic() +
    scale_color_manual(values = line.dot.colours) +
    scale_y_continuous(limits = c(-0.5,6), expand = c(0,0), breaks = c(0,2,4,6)) +
    ...
    labs(x="", y = "Fold enrichment of\nCHX-upregulated genes") +
    geom_text(data = labels, aes(label = groupings), x=4.8)
...
figs2d <- cds.event.time %>% 
    ...
    scale_fill_manual(values = fill.colours)+
    scale_y_continuous(limits = c(-80,2000), expand = c(0,0), breaks = c(500*(0:5))) +
    ...
    ggtitle("CHX-regulated AS") +
    annotate("segment", x=0.35, xend=0.35, y = 0, yend = 2000, size = 0.75) +
...
### Conservation scores on groupings above
...
figs2f <- flow1 + plot.list[[1]] + plot.list[[2]]
figs2f
...
figs2g <- flow2 + plot.list[[3]] + plot.list[[4]]
figs2g

figs2fg <- wrap_plots(figs2f,figs2g)+plot_layout(nrow=1)
figs2fg
```
...
plot1 <- figs2a+figs2c + plot_layout(widths = c(2,1))
plot2 <- figs2d/figs2e
plot2 <- wrap_plots(figs2b, plot2) + plot_layout(ncol=2,widths = c(2,1))

figs2 <- plot1/plot2/figs2fg + 
    plot_layout(heights = c(.85,1.2,.5)) + 
    plot_annotation(tag_levels = list(c("A","C","B","D","E","F","","","G","",""))) &
    theme(plot.tag = element_text(size = 20, face="bold"))
...
dat <- cds.event.time %>% 
    arrange(gene_id, desc(ds.CHX.sig), desc(factR.event), desc(ASNMDtype)) %>% 
    distinct(Timepoint,gene_id, .keep_all = TRUE) %>% 
    filter(ds.CHX.sig|!is.na(ds.CHX.sig), dge.CHX.up, psi.gex.sig) %>% 
    replace_na(list(ds.CHX.sig = FALSE)) %>% 
    mutate(Type = ifelse(Type == "CE", "CE", "Other")) %>% 
    mutate(ASNMDtype = ifelse(ASNMDtype == "Stimulating", "NS","NR")) %>% 
...
dat.kendall <- dat %>% 
    ...
    # group_by(gene.cor.bin, groupings) %>% tally() %>% 
    # group_by(groupings) %>% 
    # mutate(n = n/sum(n)) 
...
figs4 <- do.call(wrap_plots,lapply(paste0("Rank ", 1:28) , function(r){
    r.dat <- dat %>%
...
figs5a <- do.call(wrap_plots,lapply(names(names), function(grp){
    dat.kendall %>%
        filter(groupings == grp) %>% 
        # mutate(n = n*100) %>%
...
figs5bc <- do.call(wrap_plots, lapply(names(names), function(dir){
    dat.grouped  %>% 
...
figs5de <- do.call(wrap_plots, lapply(names(names), function(d){
    dat.ase%>% 
        ...
        theme(legend.position = "none",
          ...
          plot.title = element_text(hjust=0.5)) +
        labs(x="", y = "PhastCons score")+
        scale_x_discrete(labels = c("Non-reg", "Non-CE",
                                    "NR-CE", "NS-CE")) +
...
selected.genes <- c("Ripk1", "Iqgap1", "Ptbp1", "Fmnl3", 
                    "Ptbp2", "Dlg4","Gabbr1","Stx1b", "Vamp2")
selected.genes.meta <- genes.meta[genes.meta$gene_name %in% selected.genes,]
selected.genes.meta <- as.data.frame(selected.genes.meta)
rownames(selected.genes.meta) <- selected.genes.meta$gene_name
selected.genes.meta <- selected.genes.meta[selected.genes,]
...
figs5f <- Heatmap(dat, name = " ", cluster_rows = F, cluster_columns = F, 
        col = colorRampPalette(rev(brewer.pal(n = 9, name = "RdYlBu")))(400),
        column_split = factor(rep(unique(dds$day), each=2),
                              unique(dds$day)),
        column_names_rot = 0)
figs5f <- grid.grabExpr(draw(figs5f, padding = unit(c(20,10,20,10), "mm")))
...
ggsave(plot = figs5, filename = "../data/figures/FigS5.pdf", device = "pdf", 
       units = "mm", width = 200, height = 200, dpi = 30, scale = 1.8)
```
...
# load data
hipp.ase.meta <- read_tsv("../../outputs/tables/Linnarsson_factR_ase2.tsv.gz")
gex.kendall <- read_tsv("../data/tables/gex_hippocampus_kendall4.tsv.gz")
...
dat <- dat %>% 
    ...
    bind_rows(dat %>% filter(factR.event & gene.cor.estimate>0 & gene.cor.pval<0.05)) %>% 
...
stats <- dat.frac %>% 
    ...
    mutate(bg = n[3], bgrest = rest[3]) %>% 
    rowwise() %>% 
    mutate(fisher.p = fisher.test(rbind(c(n,rest), c(bg, bgrest)))$p.value) %>% 
    ungroup() %>% 
    filter(dir != "Not-regulated") %>% 
...
dat <- dat %>% 
    distinct(gene_id, .keep_all = T) %>% 
    bind_rows(dat %>% filter(factR.event,  gex.KeP < 0.05, abs(gex.KeT) > 0.75 & gene.cor.estimate>0 & gene.cor.pval<0.05, T)) %>% 
...
dat <- cds.event.time %>% 
    filter(ds.CHX.sig, psi.gex.sig, !is.na(rank_by_KeT)) %>%
    left_join(ds.chx) %>% 
...
dat.ptb %>% 
    ...
    mutate(across(.cols = c(PhastCons.score,psi.gex.PeT, psi.gex.PeP, gex.KeT, gex.KeP, psi.KeT, psi.KeP),
           formatC, digits=3)) %>% 
    dplyr::select(out.cols) %>% 
        write.xlsx("../data/supptables/TableS3.xlsx")
```
...
dat <- dat %>% filter(factR.event & gene.cor.estimate>0 & gene.cor.pval<0.05) %>% 
    filter(AStype == "CE", ASNMDtype == "Stimulating") %>% 
    filter(abs(gex.KeT) > 0.75, gex.KeP < 0.05) %>% 
